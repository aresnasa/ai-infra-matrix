    ...
    location = /gitea/user/login {
        ...
        # Map SSO admin user to configured Gitea admin user
        if ($user_header = "admin") { set $user_header "${GITEA_ALIAS_ADMIN_TO}"; }
        set $email_header $auth_email_webauth;
        if ($email_header = "") { set $email_header $auth_email; }
        
        # 调试输出
        add_header X-Debug-Auth-User-Raw "$auth_user" always;
        add_header X-Debug-Auth-Email-Raw "$auth_email" always;
        add_header X-Debug-Auth-User-Webauth "$auth_user_webauth" always;
        add_header X-Debug-Auth-Email-Webauth "$auth_email_webauth" always;
    ...
    # 处理/gitea/根路径 - 直接代理到Gitea主页，让iframe正常显示
    location = /gitea/ {
