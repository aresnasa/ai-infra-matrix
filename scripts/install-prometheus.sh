...
此脚本已预配置以下默认值:
  Prometheus 版本: {{PROMETHEUS_VERSION}}
...
示例:
    ...
    # 使用 curl 管道安装
    curl -fsSL http://{{EXTERNAL_HOST}}:{{APPHUB_PORT}}/scripts/install-prometheus.sh | bash
EOF
    exit 0
}
...
main() {
    ...
    # 验证版本
    if [[ -z "$PROMETHEUS_VERSION" ]] || [[ "$PROMETHEUS_VERSION" == "{{PROMETHEUS_VERSION}}" ]]; then
